...
class GameSpecsBottomsheet extends StatefulWidget {
  final String title;
  final TextEditingController? controller;
  final List<String>? gamesSpecs;
  final String labelText;
  const GameSpecsBottomsheet({
    super.key,
    required this.title,
    this.controller,
    this.gamesSpecs,
  ...
  @override
  State<GameSpecsBottomsheet> createState() => _GameSpecsBottomsheetState();
}

class _GameSpecsBottomsheetState extends State<GameSpecsBottomsheet> {
              ...
              SizedBox(
                ...
                child: ListWheelScrollView.useDelegate(
                  ...
                  onSelectedItemChanged: (index) {
                    setState(() {
                      selectedIndex = index;
                      widget.controller?.text = widget.gamesSpecs?[index] ?? '';
                    });
                  },
                  childDelegate: ListWheelChildBuilderDelegate(
                    childCount: widget.gamesSpecs?.length ?? 0,
                    builder: (context, index) {
                      if (index < 0 || index >= widget.gamesSpecs!.length) {
                        return null;
                      }

                      final games = widget.gamesSpecs![index];
